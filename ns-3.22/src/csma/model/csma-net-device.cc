...
CsmaNetDevice::Receive (Ptr<Packet> packet, Ptr<CsmaNetDevice> senderDevice)
{
  ...
  if (IsReceiveEnabled () == false)
    {
      NS_LOG_INFO ("Dropping pkt due to not enable ");
  ...
  if (m_receiveErrorModel && m_receiveErrorModel->IsCorrupt (packet) )
    {
      NS_LOG_INFO ("Dropping pkt due to error model ");
      m_phyRxDropTrace (packet);
      return;
    }
  ...
  NS_LOG_INFO ("Received, Pkt source is " << header.GetSource () << " destination is " << header.GetDestination ());
  //NS_LOG_INFO ("Pkt destination is " << header.GetDestination ());
...
CsmaNetDevice::SendFrom (Ptr<Packet> packet, const Address& src, const Address& dest, uint16_t protocolNumber)
{
  ...
  NS_LOG_LOGIC ("UID is " << packet->GetUid () << ")");
  NS_LOG_INFO ("Sending From : " << src << " To : " << dest );
  NS_ASSERT (IsLinkUp ());
