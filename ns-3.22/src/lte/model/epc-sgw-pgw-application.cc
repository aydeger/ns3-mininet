...
EpcSgwPgwApplication::RecvFromTunDevice (Ptr<Packet> packet, const Address& source, const Address& dest, uint16_t protocolNumber)
{
  ...
  // for simplicity, just hardcoded the UE IP address 
  // this should be replace with IP access list in the future
  Ipv4Address oldUeAddr = ueAddr;
  std::map<Ipv4Address, Ipv4Address>::iterator itx =  m_UeAccessListByDestIp.find (ueAddr);
  if (itx ==  m_UeAccessListByDestIp.end ())
    { // do nothing
    }
  else {
      ueAddr = itx->second;  
  ...
  //
  NS_LOG_INFO("previous packet addressed to " << oldUeAddr << " UE gateway " << ueAddr ); 
  ...
  if (it == m_ueInfoByAddrMap.end ())
    {        
      ...
      // check the IP access list that content UE address and any allowable IP addresses
      // where the UE acts as the gateway
      // IP access list format, UE IP address, IP address and subnet
      // then find again the
    }
  else
    {
...
void 
EpcSgwPgwApplication::AddUeAccessList (Ipv4Address ip1, Ipv4Address ip2)
  {
     m_UeAccessListByDestIp.insert( std::pair<Ipv4Address,Ipv4Address>(ip1,ip2));
  }
}  // namespace ns3
